...
Class PMS_Form_Handler {
    ...
    public static function automatically_log_in( $user_data ) {
        ...
        if ( !empty( $pms_settings['general']['automatically_log_in'] ) && !empty( $user_data['user_login'] ) && !empty( $user_data['user_pass'] ) ){

            if ( !empty( $pms_settings['payments']['gateways']['paypal']['reference_transactions'] ) && !empty( $_POST['pay_gate'] ) && ($_POST['pay_gate'] == 'paypal_express') )
                return;
