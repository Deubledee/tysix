...
</iron-iconset-svg>`;document.head.appendChild(template$4.content);const PaperDialogBehaviorImpl={hostAttributes:{role:"dialog",tabindex:"-1"},properties:{modal:{type:Boolean,value:!1},__readied:{type:Boolean,value:!1}},observers:["_modalChanged(modal, __readied)"],listeners:{tap:"_onDialogClick"},ready:function(){this.__prevNoCancelOnOutsideClick=this.noCancelOnOutsideClick;this.__prevNoCancelOnEscKey=this.noCancelOnEscKey;this.__prevWithBackdrop=this.withBackdrop;this.__readied=!0},_modalChanged:function(modal,readied){if(!readied){return}if(modal){this.__prevNoCancelOnOutsideClick=this.noCancelOnOutsideClick;this.__prevNoCancelOnEscKey=this.noCancelOnEscKey;this.__prevWithBackdrop=this.withBackdrop;this.noCancelOnOutsideClick=!0;this.noCancelOnEscKey=!0;this.withBackdrop=!0}else{this.noCancelOnOutsideClick=this.noCancelOnOutsideClick&&this.__prevNoCancelOnOutsideClick;this.noCancelOnEscKey=this.noCancelOnEscKey&&this.__prevNoCancelOnEscKey;this.withBackdrop=this.withBackdrop&&this.__prevWithBackdrop}},_updateClosingReasonConfirmed:function(confirmed){this.closingReason=this.closingReason||{};this.closingReason.confirmed=confirmed},_onDialogClick:function(event){for(var path=dom(event).path,i=0,l=path.indexOf(this),target;i<l;i++){target=path[i];if(target.hasAttribute&&(target.hasAttribute("dialog-dismiss")||target.hasAttribute("dialog-confirm"))){this._updateClosingReasonConfirmed(target.hasAttribute("dialog-confirm"));this.close();event.stopPropagation();break}}}},PaperDialogBehavior=[IronOverlayBehavior,PaperDialogBehaviorImpl];var paperDialogBehavior={PaperDialogBehaviorImpl:PaperDialogBehaviorImpl,PaperDialogBehavior:PaperDialogBehavior};const $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML=`<dom-module id="paper-dialog-shared-styles">
  <template>
    <style>
      :host {
        display: block;
...
`,is:"paper-dialog-scrollable",properties:{dialogElement:{type:Object}},get scrollTarget(){return this.$.scrollable},ready:function(){this._ensureTarget();this.classList.add("no-padding")},attached:function(){this._ensureTarget();requestAnimationFrame(this.updateScrollState.bind(this))},updateScrollState:function(){this.toggleClass("is-scrolled",0<this.scrollTarget.scrollTop);this.toggleClass("can-scroll",this.scrollTarget.offsetHeight<this.scrollTarget.scrollHeight);this.toggleClass("scrolled-to-bottom",this.scrollTarget.scrollTop+this.scrollTarget.offsetHeight>=this.scrollTarget.scrollHeight)},_ensureTarget:function(){this.dialogElement=this.dialogElement||this.parentElement;if(this.dialogElement&&this.dialogElement.behaviors&&0<=this.dialogElement.behaviors.indexOf(PaperDialogBehaviorImpl)){this.dialogElement.sizingTarget=this.scrollTarget;this.scrollTarget.classList.remove("fit")}else if(this.dialogElement){this.scrollTarget.classList.add("fit")}}});Polymer({_template:html`
    ...
    <slot></slot>
`,is:"paper-dialog",behaviors:[PaperDialogBehavior,NeonAnimationRunnerBehavior],listeners:{"neon-animation-finish":"_onNeonAnimationFinish"},_renderOpened:function(){this.cancelAnimation();this.playAnimation("entry")},_renderClosed:function(){this.cancelAnimation();this.playAnimation("exit")},_onNeonAnimationFinish:function(){if(this.opened){this._finishRenderOpened()}else{this._finishRenderClosed()}}});Polymer({is:"paper-icon-button-light",_template:html`
    <style>
      :host {
        display: inline-block;
        position: relative;
...
`,is:"paper-textarea",behaviors:[PaperInputBehavior,IronFormElementBehavior],properties:{_ariaLabelledBy:{observer:"_ariaLabelledByChanged",type:String},_ariaDescribedBy:{observer:"_ariaDescribedByChanged",type:String},value:{type:String},rows:{type:Number,value:1},maxRows:{type:Number,value:0}},get selectionStart(){return this.$.input.textarea.selectionStart},set selectionStart(start){this.$.input.textarea.selectionStart=start},get selectionEnd(){return this.$.input.textarea.selectionEnd},set selectionEnd(end){this.$.input.textarea.selectionEnd=end},_ariaLabelledByChanged:function(ariaLabelledBy){this._focusableElement.setAttribute("aria-labelledby",ariaLabelledBy)},_ariaDescribedByChanged:function(ariaDescribedBy){this._focusableElement.setAttribute("aria-describedby",ariaDescribedBy)},get _focusableElement(){return this.inputElement.textarea}});class cmsConfirm extends PolymerElement{static get template(){return html$1`
    ...
    `}static get is(){return"cms-confirm"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker}},confirm:{type:Boolean,notify:!0,value:!1,reflectToAttribute:!0},open:{type:Boolean,notify:!0,value:!1},title:{type:Object,notify:!0,observer:"cleanUnderscore"},type:{type:String},method:{type:Object}}}log(data){console.log("log from cms-confirm",data)}error(data){console.error("error from cms-page-viewer",data)}delete(){this.method(this.title);this.open=!this.open;this.confirm=!1}cleanUnderscore(data){let cleaned=data;cleaned=cleaned.split("_").join(" ");return cleaned}ready(){super.ready()}openConfirm(event){if(!1===this.confirm){this.title=this.cleanUnderscore(event.name);this.confirm=!0;this.$.animated.open()}else{this.$.animated.cancel();setTimeout(()=>{this.open=!1;this.confirm=!1},400)}}}customElements.define(cmsConfirm.is,cmsConfirm);const __DEV=!0,_DBW=new dataBaseworker,_STYLES=_DBW.getElementAssets("cms-controler",__DEV);setPassiveTouchGestures(!0);setRootPath("/");class cmsControler extends PolymerElement{static get template(){return html$1`
      <custom-style include="cms-common-styles_v2">
        <style> 
        app-header,
        nav[toolbar] {
        ...
        app-header,
        app-header-layout {
          left: var(--app-header-leftposition)
        }
        ...
        app-header {
          color: var(--app-secondary-text-color);
          right: var(--app-header-rightposition)
        }
      ...
      <shop-category-data lang="{{lang}}">
          ...
          <app-header-layout>
                ...
                <iron-selector selected="[[page]]" attr-for-selected="name" class="sellector-list" role="navigation"> 
                  <div class="content-wrapper"> 
                    ...
                    <nav>      
                      ...
                      <div>
                        <a name="media" href="[[rootPath]]media/search">[[galleries]]</a>
                      </div>                
                    </nav>    
                  </div>                  
                </iron-selector>           
            ...
            <iron-pages selected="[[page]]" attr-for-selected="name">
              ...
              <cms-image-viewer name="media" route="[[subroute]]" user="[[user]]" lang="[[lang]]"> 
              </cms-image-viewer>
    ...
    <cms-confirm id="confirm" bottom2 open="{{confirm}}" type="gallery"> 
        ...
        `}static get is(){return"cms-controler"}static get properties(){return{user:{type:Object,notify:!0},stylesSet:{type:Boolean,notify:!0,value:!1},lang:{type:String,notify:!0,observer:"__changeLang"},langs:{type:Object,notify:!0,observer:"_setLang"},openMain:{type:Boolean,notify:!0},page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},open:{type:Boolean,value:!1},categories:{type:Array,notify:!0},confirm:{type:Boolean,notify:!0,value:!1},routeData:Object,subroute:Object}}static get observers(){return["_routePageChanged(routeData.page, route)"]}static connectedCallback(){console.log("aloo")}ready(){super.ready();this.addEventListener("confirm",this.openConfirm);_STYLES.then(querySnapshot=>{let style=querySnapshot.data();this._setLangObject(style)}).catch(function(error){console.error("Error reteaving assets: ",error)})}__changeLang(){if(this.langs[this.lang]){window.localStorage.setItem("lang",this.lang);let obj=this.langs[this.lang];for(let par in obj){this.set(par,obj[par])}}}_setLangObject(langs){let obj={};for(let par in langs){if("styles"!==par){obj[par]=langs[par].pop()}}this.set("langs",obj)}_setLang(){let lang=window.localStorage.getItem("lang");if(!0===!!lang){if(0<lang.split("").length){this.set("lang",lang)}}else{this.set("lang","en")}}openConfirm(event){this.$.confirm.openConfirm({name:event.detail.name});this.$.confirm.method=event.detail.method;this.confirm=!this.confirm}_routePageChanged(page){if(!page){this.page=""}else if(-1!==["app","content","users","home","media"].indexOf(page)){this.page=page}else{this.page="view404"}}_pageChanged(page){if("home"===page){import("./cms-home-viewer.js").then(bundle=>bundle&&bundle.$cmsHomeViewer||{});return}if("content"===page){import("./cms-content.js").then(bundle=>bundle&&bundle.$cmsContent||{});return}if("users"===page){import("./cms-user-viewer.js").then(bundle=>bundle&&bundle.$cmsUserViewer||{});return}if("media"===page){import("./cms-image-viewer.js").then(bundle=>bundle&&bundle.$cmsImageViewer||{});return}if("view404"===page){import("../shop-404-warning.js").then(bundle=>bundle&&bundle.$shop$404Warning||{});return}}}customElements.define(cmsControler.is,cmsControler);export{appLayoutBehavior as $appLayoutBehavior,appRouteConverterBehavior as $appRouteConverterBehavior,paperDialogBehavior as $paperDialogBehavior,AppLayoutBehavior,AppRouteConverterBehavior,PaperDialogBehaviorImpl,PaperDialogBehavior};