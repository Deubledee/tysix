...
</iron-iconset-svg>`;document.head.appendChild(template$1.content);const template$2=html`<iron-iconset-svg name="icons" size="24">
<svg><defs>
...
<dom-module id="paper-material-styles">
  ...
  <slot></slot>`;template$5.setAttribute("strip-whitespace","");Polymer({_template:template$5,is:"paper-button",behaviors:[PaperButtonBehavior],properties:{raised:{type:Boolean,reflectToAttribute:!0,value:!1,observer:"_calculateElevation"}},_calculateElevation:function(){if(!this.raised){this._setElevation(0)}else{PaperButtonBehaviorImpl._calculateElevation.apply(this)}}});const $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML=`<iron-iconset-svg name="paper-dropdown-menu" size="24">
<svg><defs>
<g id="arrow-drop-down"><path d="M7 10l5 5 5-5z"></path></g>
</defs></svg>
</iron-iconset-svg>`;document.head.appendChild($_documentContainer.content);const template$6=html`
<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;template$7.setAttribute("style","display: none;");document.head.appendChild(template$7.content);const $_documentContainer$1=document.createElement("template");$_documentContainer$1.setAttribute("style","display: none;");$_documentContainer$1.innerHTML=`<dom-module id="paper-dropdown-menu-shared-styles">
  <template>
    <style>
      :host {
        display: inline-block;
...
</custom-style>`;template$8.setAttribute("style","display: none;");document.head.appendChild(template$8.content);const PaperInputAddonBehavior={attached:function(){this.fire("addon-attached")},update:function(state){}};var paperInputAddonBehavior={PaperInputAddonBehavior:PaperInputAddonBehavior};Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
        float: right;
...
`;template$a.setAttribute("strip-whitespace","");Polymer({_template:template$a,is:"paper-spinner",behaviors:[PaperSpinnerBehavior]});Polymer({_template:html`
    <style>
      :host {
        @apply --layout-inline;
        @apply --layout-center;
...
</dom-module>`;document.head.appendChild($_documentContainer$4.content);const template$c=html`
...
</custom-style>`;template$c.setAttribute("style","display: none;");document.head.appendChild(template$c.content);const $_documentContainer$5=document.createElement("template");$_documentContainer$5.innerHTML=`
<dom-module id="cms-common-top-styles">
  <template>
    ...
    main {
       ...
       /* position: var(--app-custom-position);
        top: var(--app-content-top);
        width: var(--app-default-width);*/
    ...
    article[centerListItem] {
        flex-flow: var(--app-flexwrap);
        letter-spacing: var(--app-content-letter-spacing);
        font-size: var(--app-content-font-size);
        padding: var(--app-default-min-padding);
        margin-top: var(--app-content-margin-top);
        margin-bottom: var(--app-content-margin-bottom);
        margin-left: var(--app-content-article-margin-left);
        max-width: var(--app-default-width)
    }

    article[centerListItem] div {
        flex-basis: var(--app-content-article-lsitItem-flex-basis);
...
</dom-module>`;document.head.appendChild($_documentContainer$5.content);const $_documentContainer$6=document.createElement("template");$_documentContainer$6.innerHTML=`
    ...
    <template>
        ...
        display: var(--app-block);
            ...
            div[table]::-webkit-scrollbar {
                width: var(--app-content-margin-left)
            }
...
</dom-module>`;document.head.appendChild($_documentContainer$6.content);class worker{constructor(){this.db=firebase.firestore();this.auth=firebase.auth}getDoc(table){var categoriesRef=this.db.collection(table.name).doc(table.doc);return categoriesRef.get()}getDocList(table){var pagesRef=this.db.collection(table.name);return pagesRef.get()}queryDocList(table){var pagesRef=this.db.collection(table.name).where(table.query,table.condition,table.value);return pagesRef.get()}createDoc(table){var pagesRef=this.db.collection(table.name).doc(table.docName);return pagesRef.set(table.doc)}deleteDoc(table){var pagesRef=this.db.collection(table.name).doc(table.docName);return pagesRef.delete()}getDocListDev(table){var pagesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name);return pagesRef.get()}getDocDev(table){var categoriesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name).doc(table.doc);return categoriesRef.get()}queryDocListDev(table){var pagesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name).where(table.query,table.condition,table.value);return pagesRef.get()}createDocDev(table){var pagesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name).doc(table.docName);console.log(pagesRef);return pagesRef.set(table.doc)}deleteDocDev(table){var pagesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name).doc(table.docName);return pagesRef.delete()}updateContent(done,table){var imagesRef=this.db.collection(table.name).doc(table.doc);return imagesRef.update(table.data)}updateContentDev(done,table){var imagesRef=this.db.collection("dev").doc("VoSSMkzGYmPTvUhh9mgL").collection(table.name).doc(table.doc);return imagesRef.update(table.data)}mailLink(){if(this.auth().isSignInWithEmailLink(window.location.href)){var email=window.localStorage.getItem("emailForSignIn");if(!email){email=window.prompt("Please provide your email for confirmation")}this.auth().signInWithEmailLink(email,window.location.href).then(function(result){window.localStorage.removeItem("emailForSignIn")}).catch(function(error){})}}authState(done){this.auth().onAuthStateChanged(user=>{if(user){let obj={name:"users",doc:user.uid};this.getDoc(obj).then(querySnapshot=>{let user=querySnapshot.data();done(user)}).catch(function(error){console.error("Error reteaving article: ",error);done("error",error)})}else{done(0,"No user is signed in.")}})}login(user){this.auth().signInWithEmailAndPassword(user.email,user.pwd).then(item=>{}).catch(function(error){var errorCode=error.code,errorMessage=error.message;console.log(errorMessage,errorCode)})}andleTransaction(table){var sfDocRef=this.db.collection(table.name).doc(table.doc);sfDocRef.set(table.query);return this.db.runTransaction(function(transaction){return transaction.get(sfDocRef).then(function(sfDoc){if(!sfDoc.exists){throw"Document does not exist!"}var newPopulation=sfDoc.data()})}).then(function(){console.log("Transaction successfully committed!")}).catch(function(error){console.log("Transaction failed: ",error)})}}var firebaseWorker={worker:worker};const Worker=new worker;class dataBaseworker{constructor(){this.categories=[]}updateArticles(done,table,dev){let teble={name:"articles",doc:table.name,data:{content:table.content}};if(!1===dev){Worker.updateContent(done,teble).then(function(){console.log("gallerie successfully updated!");done("gallerie successfully updated!",table.gallerie)}).catch(function(error){done("Error",error)})}else{Worker.updateContentDev(done,teble).then(function(){console.log("gallerie successfully updated!");done("gallerie successfully updated!",table.gallerie)}).catch(function(error){done("Error",error)})}}setArticles(done,table,dev){let obj=table.parent!==void 0?{name:"articles",docName:table.parent,doc:table}:!1;if(!1===obj){done(!1,!1);return}if(!1===dev){Worker.createDoc(obj).then(function(){done("newPage")}).catch(error=>{done("error",error);console.error("Error writing document: ",error)})}else{Worker.createDocDev(obj).then(function(){done("newPage")}).catch(error=>{done("error",error);console.error("Error writing document: ",error)})}}deleteArticles(done,page,dev){if(!1===dev){Worker.deleteDoc({name:"articles",docName:page}).then(function(){done("Page successfully deleted!")}).catch(function(error){console.error("Error removing article: ",error);done("error",error)})}else{Worker.deleteDocDev({name:"articles",docName:page}).then(function(){done("Page successfully deleted!")}).catch(function(error){console.error("Error removing article: ",error);done("error",error)})}}getArticle(done,categoryObj,dev){let obj={name:"articles",doc:categoryObj.name};if(!1===dev){Worker.getDoc(obj).then(querySnapshot=>{let content=querySnapshot.data().content;done("categorie",content)}).catch(function(error){console.error("Error reteaving article: ",error);done("error",error)})}else{Worker.getDocDev(obj).then(querySnapshot=>{let content=querySnapshot.data().content;done("categorie",content)}).catch(function(error){console.error("Error reteaving article: ",error);done("error",error)})}}askAllArticles(done,dev){let obj={name:"articles"};if(!1===dev){Worker.getDocList(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting All Articles: ",error);done("error",error)})}else{Worker.getDocListDev(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting All Articles: ",error);done("error",error)})}}writePagesContent(done,table,dev){let teble={name:"pages",doc:table.name,data:table};console.log(teble);if(!1===dev){Worker.updateContent(done,teble).then(function(){console.log("page successfully updated!");done("page successfully updated!",table.name)}).catch(function(error){done("Error",error)})}else{Worker.updateContentDev(done,teble).then(function(){console.log("page successfully updated!");done("page successfully updated!",table.name)}).catch(function(error){done("Error",error)})}}setPages(done,parsed,dev){console.log(parsed);let obj=parsed.name!==void 0?{name:"pages",docName:parsed.name,doc:parsed}:!1;if(!1===obj){done(!1,!1);return}if(!1===dev){Worker.createDoc(obj).then(function(){done("newPage")}).catch(error=>{done("error",error);console.error("Error writing page: ",error)})}else{Worker.createDocDev(obj).then(function(){done("newPage")}).catch(error=>{done("error",error);console.error("Error writing page: ",error)})}}deletePage(done,page,dev){if(!1===dev){Worker.deleteDoc({name:"pages",docName:page}).then(function(){done("Page successfully deleted!",page)}).catch(function(error){console.error("Error removing page: ",error);done("error",error)})}else{Worker.deleteDocDev({name:"pages",docName:page}).then(function(){done("Page successfully deleted!",page)}).catch(function(error){console.error("Error removing page: ",error);done("error",error)})}}askAllPages(done,dev){let obj={name:"pages"};if(!1===dev){Worker.getDocList(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting all pages: ",error);done("error",error)})}else{Worker.getDocListDev(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting all pages: ",error);done("error",error)})}}getPagesEqualTo(done,query,value,dev){let obj={name:"pages",query:query,value:value,condition:"=="};this.categories=[];if(!1===dev){Worker.queryDocList(obj).then(querySnapshot=>{querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done({categories:this.categories})}).catch(function(error){console.error("Error getting Pages Equal To: "+value,error);done("error",error)})}else{Worker.queryDocListDev(obj).then(querySnapshot=>{querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done({categories:this.categories})}).catch(function(error){console.error("Error getting Pages Equal To: "+value,error);done("error",error)})}}writeImageContent(done,table,dev){let teble={name:"images",doc:table.gallerie,data:{content:table.content}};if(!1===dev){Worker.updateContent(done,teble).then(function(){console.log("gallerie successfully updated!");done("gallerie successfully updated!",table.gallerie)}).catch(function(error){done("Error",error)})}else{Worker.updateContentDev(done,teble).then(function(){console.log("gallerie successfully updated!");done("gallerie successfully updated!",table.gallerie)}).catch(function(error){done("Error",error)})}}getImageGalleries(done,dev){let obj={name:"images"};if(!1===dev){Worker.getDocList(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting Image Galleries ",error);done("error",error)})}else{Worker.getDocListDev(obj).then(querySnapshot=>{this.categories=[];querySnapshot.forEach(doc=>{this.categories.push(doc.data())});done(this.categories)}).catch(function(error){console.error("Error getting Image Galleries ",error);done("error",error)})}}setImageGalleries(done,table,dev){let obj={name:String,docName:String,doc:Object,name:"images",docName:table.gallerie,doc:table.content};if(!1===dev){Worker.createDoc(obj).then(()=>{done("gallerie created",obj.docName)}).catch(error=>{console.error("Error creating gallerie"+error);done("error",error)})}else{Worker.createDocDev(obj).then(()=>{done("gallerie created",obj.docName)}).catch(error=>{console.error("Error creating gallerie"+error);done("error",error)})}}deleteGallerie(done,gallerie,dev){if(!1===dev){Worker.deleteDoc({name:"images",docName:gallerie+"_images"}).then(function(){done("Gallerie successfully deleted!",gallerie)}).catch(function(error){console.error("Error deleting gallerie"+error);done("error",error)})}else{Worker.deleteDocDev({name:"images",docName:gallerie+"_images"}).then(function(){done("Gallerie successfully deleted!",gallerie)}).catch(function(error){console.error("Error deleting gallerie"+error);done("error",error)})}}loginFire(user){Worker.login(user)}authStateChanged(done){Worker.authState(done)}checkMailLink(){Worker.mailLink()}getElementAssets(element,dev){let obj={name:"stylesandlangs",doc:element};if(!1===dev){return Worker.getDoc(obj)}else{return Worker.getDocDev(obj)}}setElementAssets(table,dev){let obj={name:"stylesandlangs",docName:String,doc:Object,docName:table.element,doc:table.content};if(!1===dev){return Worker.createDoc(obj)}else{return Worker.createDocDev(obj)}}}var dataBaseWorker={dataBaseworker:dataBaseworker};class cmsGallerieForm extends PolymerElement{static get template(){return html`
...
`}static get is(){return"cms-gallerie-form"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker}},closed:{type:Boolean,notify:!0,value:!1,reflectToAttribute:!0},setter:{type:String,notify:!0,value:"false"},pageName:{type:String}}}ready(){super.ready()}setValues(){if(1<this.pageName.length){let parsed={gallerie:this.pageName+"_images",content:{content:[{url:"data:image/svg+xml,"+encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\"><path fill=\"#CCC\" d=\"M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z\"/></svg>"),title:""}],gallerie:this.pageName}};this.DBW.setImageGalleries((msg,gallerie)=>{console.log(msg,gallerie);this.clean("true")},parsed)}}clean(setterValue){let setter;if(!0===setterValue instanceof MouseEvent){setter="true"}else{setter=setterValue}this.closed=!1;this.setter=setter}}customElements.define(cmsGallerieForm.is,cmsGallerieForm);class cmsGalleriesItem extends PolymerElement{static get template(){return html` 
        <style>
            .title {
                font-weight: bold;
                font-size: 25px;
        ...
        `}static get is(){return"cms-gallerie-item"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker},notify:!0},openMain:{type:Boolean,notify:!0,value:!0},setter:{type:String,notify:!0,value:"false"},sett:{type:Boolean,value:!0},setg:{type:Object,notify:!0},gallerie:{type:Array,notify:!0,observer:"setGallerie"}}}ready(){super.ready()}setGallerie(data,ain){let again=ain||0;if(data.content[0]!==void 0){let img=new Image;img.crossOrigin="Anonymous";img.onload=event=>{this.$.img.crossOrigin="Anonymous";this.$.img.src=img.src;this.$.galleries.style.backgroundImage=`url(${img.src})`};img.onerror=event=>{this._checkDataPlease(data,again);console.info("Smile Tho' your heart is ! Smile Even tho' it's breaking... :)")};try{if(0<=data.content.length){img.src=0===again?data.content[0].url:data.content[again].url;img.onload();return}}catch(err){console.info("strumming my pain with his fingers.. killing me softly with his song..! :)")}}}_checkDataPlease(data,again){if(0<data.content.length){again=again+1;this.setGallerie(data,again)}}settg(){this.setg=this.gallerie}addImage(){this.dispatchEvent(new CustomEvent("add-image",{bubbles:!0,composed:!0,detail:{imageArray:this.gallerie.content,gallerie:this.gallerie.gallerie}}))}deleteGallerie(data){this.DBW.deleteGallerie((done,err)=>{if("error"!==done){this.setter=!0}else{console.error(err)}},data)}openConfirm(event){this._changeSectionDebouncer=Debouncer.debounce(this._changeSectionDebouncer,microTask,()=>{this.dispatchEvent(new CustomEvent("confirm",{bubbles:!0,composed:!0,detail:{name:this.gallerie.gallerie,method:this.deleteGallerie.bind(this)}}))})}}customElements.define(cmsGalleriesItem.is,cmsGalleriesItem);class cmsGalleries extends PolymerElement{static get template(){return html` 
        <style>        
            nav[galleries] {
               /* overflow-y: auto;
        ...
        `}static get is(){return"cms-galleries"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker},notify:!0},images:{type:Array,notify:!0},setter:{type:String,notify:!0,value:"false"},confirm:{type:Boolean,value:!0},reset:{type:String,observer:"getImageGalleries",value:"false"},settg:{type:Object,notify:!0,value:{},observer:"setg"},sett:{type:Boolean,value:!0},galleries:{type:Array,notify:!0}}}ready(){super.ready();this.getImageGalleries(!0)}log(data){console.log(data)}eventFunction(event){this.reset="false";this.set("galleries",event)}getImageGalleries(data){if(!0===data||"true"===data){this.DBW.getImageGalleries(done=>{this.eventFunction(done);this.reset=!this.reset})}}setg(event){if(event.content){this.settg={};this.images=event.content;this.show=!0}else{this.images={}}}}customElements.define(cmsGalleries.is,cmsGalleries);class ShopImage extends PolymerElement{static get template(){return html`
    <style>
...
`}static get is(){return"shop-image"}static get properties(){return{alt:String,src:{type:String,observer:"_srcChanged"},placeholderImg:{type:String,observer:"_placeholderImgChanged"}}}_srcChanged(src){this.$.img.removeAttribute("src");this.$.img.style.transition="";this.$.img.style.opacity=0;if(src){this.$.img.src=src}}_onImgLoad(){this.$.img.style.transition="0.5s opacity";this.$.img.style.opacity=1}_onImgError(){if(!this.placeholderImg){this.$.img.src="data:image/svg+xml,"+encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\"><path fill=\"#CCC\" d=\"M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z\"/></svg>")}}_placeholderImgChanged(placeholder){this.style.backgroundImage="url('"+placeholder+"')"}}customElements.define(ShopImage.is,ShopImage);class cmsImages extends PolymerElement{static get template(){return html` 
    ...
    <h1>[[loading]]</h1>
        ...
        `}static get is(){return"cms-images"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker},notify:!0},cancel:{type:Boolean,notify:!0},killSett:{type:Boolean,notify:!0,observer:"settAndFormKiller"},loading:{type:String},openMain:{type:Boolean,notify:!0,value:!0},closeMethod:{type:Object,notify:!0},confirm:{type:Boolean,notify:!0,value:!1,observer:"clearImages"},setButton:{type:Boolean,notify:!0,value:!1},sett:{type:Boolean,notify:!0,value:!1},image:{type:Object,notify:!0,value:{}},images:{type:Array,notify:!0,value:[],observer:"open"},clear:{type:Boolean,value:!1,notify:!0},remove:{type:Object,notify:!0},del:{type:Boolean,notify:!0,value:!1},form:{type:Boolean,notify:!0,value:!0},show:{type:Boolean,notify:!0,value:!1,reflectToAttribute:!0},showTop:{type:Boolean,notify:!0,value:!1},toggle:{type:Boolean,notify:!0,value:!0,reflectToAttribute:!0},contents:{type:Array,notify:!0}}}ready(){super.ready()}log(data){console.log(data)}closeMethod(data){console.log(data)}settAndFormKiller(data){this.sett=!data;this.form=!0;this.del=!0;this.setButton=!0}killSpinner(contents,index){if(contents.length===index+1){this.$.spinner1.active=!1;this.loading=""}}clearImages(){if(!1===this.$.spinner1.active){this.images=[];this.contents=[];this.set("show",!1);this.clear=!this.clear;this.showTop=!1;this.$.spinner1.active=!0;this.$.main.classList.remove("mainish");this.closeMethod(!1)}}open(data){if(!0===data instanceof Array&&0<data.length){this.loading=`loading ${data.length} images... Please Waitt!!`;if(!0===this.form){this.show=!0}else{this.show=!1;this.showTop=!0}this.$.main.classList.add("mainish");this.contents=[];setTimeout(()=>{this.contents=data},60)}}close(){this.clearImages();this.cancel=!0}deleteImg(data){console.log(data.model.__data.image)}setImage(event){this.image=event.model.__data.image;event.srcElement.style.filter="drop-shadow(8px 8px 10px gray)";if(!0===this.sett||!0===this.setButton){this.set("image",event.model.__data.image)}}toggleView(){this.toggle=!this.toggle}}customElements.define(cmsImages.is,cmsImages);class cmsGalleryViewer extends PolymerElement{static get template(){return html`
        <style>
        :host {
            text-align: center;
            z-index: 122;
      ...
      <main mainOpend$="[[openMain]]">
        ...
        </cms-images>`}static get is(){return"cms-gallery-viewer"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker},notify:!0},galleries:{type:Array,notify:!0},mainOpend:{type:Boolean,notify:!0,value:!0,reflectToAttribute:!0},doNotOpenMain:{type:Boolean,notify:!0,value:!1},openMain:{type:Boolean,notify:!0,value:!0},images:{type:Array,notify:!0,observer:"opensie"},confirm:{type:Boolean,notify:!0,value:!1},reset:{type:Boolean,notify:!0},clear:{type:Boolean,value:!1,notify:!0,observer:"opensie"},show:{type:Boolean,notify:!0,value:!1,observer:"showButtons"},evet:{type:Boolean,value:!1,notify:!0,observer:"cancel"},image:{type:Object,notify:!0,observer:"addMethod"},gallerie:{type:String,notify:!0},killFormAndSet:{type:Boolean,notify:!0,value:!1,observer:"settAndFormKiller"},form:{type:Boolean,notify:!0,value:!0},sett:{type:Boolean,notify:!0,value:!1},open:{type:Boolean,notify:!0,value:!0},closed:{type:Boolean,notify:!0,value:!0,reflectToAttribute:!0}}}ready(){super.ready()}log(msg,data){console.log(msg,data)}showButtons(data){this.$.images.form=!1;this.$.images.sett=data}addMethod(data){console.log(data)}opensie(data){if(!0===data instanceof Array){this.$.images.style.display="block";this.$.images.style.height="600px";this.$.images.settAndFormKiller(!0);this.$.images.set("sett",!0);this.set("activeData",data);setTimeout(()=>{this.$.images.set("images",data)},200)}if(!1===data){this.$.images.style.display="none";this.$.images.style.height="0px";this.clear=!this.clear}}settAndFormKiller(data){if(!0===data){this.$.images.settAndFormKiller(data)}}error(data){console.error("error from cms-image-viewer",data)}cancel(event){if(!0===event){this.$.images.killSettAndForm=this.killSettAndForm=!this.killSettAndForm}}}customElements.define(cmsGalleryViewer.is,cmsGalleryViewer);class cmsImage extends PolymerElement{static get template(){return html`  
        <style include="cms-common-styles">
                ...
                <main> `}static get is(){return"cms-image"}static get properties(){return{images:{type:Object,notify:!0},content:{type:Array,notify:!0,computed:"getImage(images)"}}}galleryView(){return!1}addImage(){if(1===this.childElementCount){this.children[1].children[0].addMethod=this.setImage.bind(this)}else{this.children[1].removeChild(this.children[1].children[0])}}_fromAray(data){let url,arr=[],obj2;if(1<=data.image.length){let t=new Date;for(let i=0;i<data.image.length;i++){obj2=data.image[i];arr.push(obj2)}}else{url="";obj2={url:url,title:""};arr.push(obj2)}return arr}getImage(data){if("image"in data){let arr;if(!0===data.image instanceof Array){arr=this._fromAray(data)}return arr}}setImage(data){if("url"in data){let img=new Image,arr=[];img.src=data.url;if(600>img.naturalHeight){if(!0===this.content[0].image instanceof Array){arr=this.content[0].image}else{arr.push(this.content[0].image)}arr.push(data.url);this.content[0].image=arr;this.notifyPath("content[0].image")}if(600<=img.naturalHeight){if(!0===this.content[0].largeImage instanceof Array){arr=this.content[0].largeImage}else{arr.push(this.content[0].largeImage)}arr.push(data.url);this.content[0].largeImage=arr;this.notifyPath("content[0].largeImage")}this.addingcancel=this.adding;this.adding=!this.adding;this.$.saveButton.classList.remove("diferent");this.editing=this.editing+1}}setImages(data){this.content[0].image=data.url}del(index){if(!0===this.content[0].image instanceof Array){if(0<index){this.content[0].image.splice(index,index)}else{this.content[0].image.splice(0,1)}}else{this.content[0].image=""}}deleteImg(data){if(data!==void 0){this.del(data.model.__data.index);this.cancelButton.classList.remove("diferent");this.$.saveButton.classList.remove("diferent");this.editing=this.editing+1;this.remove=void 0}}cancelImages(){this.cancelState();if(!1===this.addingcancel){this.adding=!this.adding}}}customElements.define(cmsImage.is,cmsImage);class cmsInputSellector extends PolymerElement{static get template(){return html`         
          <custom-style>
          <style is="custom-style">
               :host {
                    width: 100%;
     ...
     `}static get is(){return"cms-login"}static get properties(){return{DBW:{type:Object,value:function(){return new dataBaseworker}},closed:{type:Boolean,notify:!0,value:!0,reflectToAttribute:!0},notLogedIn:{type:Boolean,value:!1},user:{type:Object,notify:!0},email:{type:String},pwd:{type:String},type:{type:String},types:{type:Array,value:[{label:"Login types"},{id:"google",name:"google"},{id:"facebook",name:"facebook",notAtive:!1},{id:"github",name:"github",notAtive:!1},{id:"wave-shaper",name:"Video",notAtive:!1}]}}}ready(){super.ready();this.DBW.authStateChanged((user,err)=>{if(0!==user){this.closed=!this.closed;this._userAccepted(user)}else{this.set("notLogedIn",!0);console.log(err)}});window.addEventListener("update-user",event=>{let string=`http://localhost:3000/update?obj=${JSON.stringify(event.detail)}&uid=${this._getUid()}`;console.log(event.detail,string);reuest(string,items=>{console.log("update sucssessfull",JSON.parse(items));let content=JSON.parse(items);if("admin"===content.accepted){window.dispatchEvent(new CustomEvent("user-updated",{detail:this.removeSensitiveData(JSON.parse(items))}))}else{window.dispatchEvent(new CustomEvent("app-error",{detail:content.error}))}},"POST","")});window.addEventListener("ask-for-users",(event=>{let string=`http://localhost:3000/admin?uid=${this._getUid()}`;if(null!==this._getProperty("user")){console.log()}reuest(string,items=>{console.log(items);let content=JSON.parse(items);window.dispatchEvent(new CustomEvent("user-list",{detail:content}))},"POST","")}).bind(this))}_userAccepted(user){this._changeSectionDebouncer=Debouncer.debounce(this._changeSectionDebouncer,microTask,()=>{if(-1<["admin","dev","manager","advUser"].indexOf(user.role)){import("./cms-controler.js").then(bundle=>bundle&&bundle.$cmsControler||{}).then(()=>{this.set("user",user);this.children[0].set("user",user)})}})}removeSensitiveData(categories){let finalString=[],obj={};for(let par in categories){if(-1===["passwordHash","passwordSalt","metadata","providerData","tokensValidAfterTime"].indexOf(par.toString())){obj[par.toString()]=categories[par]}finalString.push(obj)}return finalString}_getUid(){let user=this._getProperty("user");return user.uid}setValues(){let obj={email:this.email,pwd:this.pwd};this.DBW.loginFire(obj);this.email="";this.pwd=""}}customElements.define(cmsLogin.is,cmsLogin);;class cmsTopPageTemplate extends PolymerElement{static get template(){return html`
