...
function validateNumber(arg, value, options) {
    ...
    if (!validateOptional(value, options)) {
        if (typeof value !== 'number' || isNaN(value)) {
            ...
            throw new Error(`Value for argument ${formatArgumentName(arg)} must be within [${min}, ${max}] inclusive, but was: ${value}`);
        }
    }
}
...
function validateEnumValue(arg, value, allowedValues, options) {
    if (!validateOptional(value, options)) {
        ...
        throw new Error(`Invalid value for argument ${formatArgumentName(arg)}. Acceptable values are: ${expectedDescription.join(', ')}`);
    }
}
