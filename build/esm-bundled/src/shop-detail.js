import{PolymerElement,html,Debouncer,microTask}from"./shop-app.js";class ShopDetail extends PolymerElement{static get template(){return html`
    ...
    `}static get is(){return"shop-detail"}static get properties(){return{item:Object,route:Object,routeData:Object,visible:{type:Boolean,value:!1},offline:{type:Boolean,observer:"_offlineChanged"},failure:Boolean}}static get observers(){return["_itemChanged(item, visible)"]}_itemChanged(item,visible){if(visible){this._itemChangeDebouncer=Debouncer.debounce(this._itemChangeDebouncer,microTask,()=>{let text=item?item.description:"";this.$.desc.innerHTML=this._unescapeText(text);this.$.quantitySelect.value="1";this.$.sizeSelect.value="M";this.dispatchEvent(new CustomEvent("change-section",{bubbles:!0,composed:!0,detail:{category:item?item.category:"",title:item?item.title:"",description:item?item.description.substring(0,100):"",image:item?this.baseURI+item.image:""}}))})}}_unescapeText(text){let elem=document.createElement("textarea");elem.innerHTML=text;return elem.textContent}_formatPrice(price){return price?"$"+price.toFixed(2):""}_addToCart(){this.dispatchEvent(new CustomEvent("add-cart-item",{bubbles:!0,composed:!0,detail:{item:this.item,quantity:parseInt(this.$.quantitySelect.value,10),size:this.$.sizeSelect.value}}))}_isDefined(item){return null!=item}_tryReconnect(){this.$.categoryData.refresh()}_offlineChanged(offline){if(!offline){this._tryReconnect()}}}customElements.define(ShopDetail.is,ShopDetail);