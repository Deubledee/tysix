...
class dataBaseworker {
    ...
    getByCollectionPath(path) {
        return getByCollectionPath.call(this, path)
    }
    getByDocPath(path) {
        return getByDocPath.call(this, path)
    }
...
// shared functions
async function getByDocPath(path) {
    this.categories = [];
    await Worker.getByDocPath(path)
        .then((querySnapshot) => {
...
async function getByCollectionPath(path) {
    this.categories = [];
    await Worker.getByCollectionPath(path)
        .then((querySnapshot) => {
            this.categories = querySnapshot
        }).catch(async function (error) {
            this.categories = 'error'
