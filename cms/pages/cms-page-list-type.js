define(["../cms-login.js"],function(_cmsLogin){"use strict";class cmsPageListItem extends _cmsLogin.cmsItemTemplate{static get template(){return _cmsLogin.html`
    <style>    
    :host {
        position: relative;
        display: block;
    ...
    <slot name="table"></slot> 
        `}static get is(){return"cms-page-list-item"}static get properties(){return{translator:{type:Object,notify:!0,value:function(){return MyAppGlobals.translator}}}}ready(){super.ready()}_putRow(data){this.translator.template.innerHTML=`
           ...
           </article>`;this.translator.clone(this);this.children[0].children[1].children[0].addEventListener("click",this.showPage.bind(this));this.children[0].children[2].children[0].addEventListener("click",this.showCats.bind(this));this.children[0].children[4].children[0].addEventListener("click",this._confirmPublish.bind(this));this.children[0].children[5].children[0].addEventListener("click",this._openConfirm.bind(this))}_getPagename(cats){return cats}error(data){console.error("error from cms-article-viewer",data)}deSpin(){this.$.spinner.active=!this.$.spinner.active}showPage(){let string=window.btoa(`${JSON.stringify(this.page)}`);window.history.pushState({},null,`content/pages/edit-category-pages?content=${string}&add=false`);window.dispatchEvent(new CustomEvent("location-changed"))}showCats(){let string=window.btoa(`${JSON.stringify(this.page)}`);window.history.pushState({},null,`content/pages/edit-subcategory-pages?content=${string}&add=false`);window.dispatchEvent(new CustomEvent("location-changed"))}__delete(data){this.translator._DBW.deletePage(msg=>{if("error"!==msg){console.log(msg);this.__reset()}else{console.error(msg)}},data,this.translator.__DEV)}__publish(data){console.log(data)}_openConfirm(){this._changeSectionDebouncer=_cmsLogin.Debouncer.debounce(this._changeSectionDebouncer,_cmsLogin.microTask,()=>{this.dispatchEvent(new CustomEvent("confirm",{bubbles:!0,composed:!0,detail:{name:this.page.items[0].categoryName,method:this.__delete.bind(this),argument:this.page.id,headderMsgKind:"delete",type:"categoryPage"}}))})}_confirmPublish(){this._changeSectionDebouncer=_cmsLogin.Debouncer.debounce(this._changeSectionDebouncer,_cmsLogin.microTask,()=>{this.dispatchEvent(new CustomEvent("confirm",{bubbles:!0,composed:!0,detail:{name:this.page.name,method:this.__publish.bind(this),argument:"!!to be done!!",headderMsgKind:"publish",type:"categoryPage"}}))})}__reset(){this._debounceEvent=_cmsLogin.Debouncer.debounce(this._debounceEvent,_cmsLogin.microTask,()=>{window.dispatchEvent(new CustomEvent("reset-list-type",{bubbles:!0,composed:!0}))})}}customElements.define(cmsPageListItem.is,cmsPageListItem);class cmsPageListType extends _cmsLogin.cmsMiddlePageTemplate{static get _getShoutAnchor(){return _cmsLogin.html`
