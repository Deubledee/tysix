define(["../../src/cms-login.js"],function(_cmsLogin){"use strict";class cmsProjects extends _cmsLogin.cmsMiddlePageTemplate{static get _getSilentAnchor(){return _cmsLogin.html`            
...
(0,_cmsLogin.afterNextRender)(this,()=>{//  projs.then(response => console.log(response))
this.spinner.active=!1})}},120)}}/*else if (reset === true) {
        this.pages = [];
        this._contentChanged()
      }*/}_contentChanged(){if("number"===typeof this.time)clearTimeout(this.time);const spinnerTemplate=()=>_cmsLogin.html$2`<paper-spinner-lite active="false" slot="spinner">`;(0,_cmsLogin.render)(spinnerTemplate(),this);if("pages"===this.routeData.page&&"/pages"===this.route.path){this.time=setTimeout(()=>{window.history.pushState({},null,`${this.rootPath}content/pages`);window.dispatchEvent(new CustomEvent("location-changed"))},500)}else{this.pages=[]}}_setAll(response){let arr=[];this.pages=[];for(let i=0;i<response.length;i++){if(!!response[i].id){let datarr=response[i].data();arr.push(datarr)}}this.set("pages",arr)}putElement(data){if("number"===typeof this.time)clearInterval(this.time);this.time=setTimeout(()=>{const pageTemplate=pages=>_cmsLogin.html$2`${pages.map((article,idx)=>{return _cmsLogin.html$2`<cms-page-list-item slot="item${idx}" .page="${article}">
                        </cms-page-list-item>`})} `;(0,_cmsLogin.render)(pageTemplate(data),this)},60)}}customElements.define(cmsProjects.is,cmsProjects);function getProjects(){return _getProjects.apply(this,arguments)}function _getProjects(){_getProjects=babelHelpers.asyncToGenerator(function*(){let proj;yield(0,_cmsLogin.request)("http://127.0.0.1:8001/data/projects.json","GET").then(res=>res.text()).then(res=>{proj=res});return proj});return _getProjects.apply(this,arguments)}function changeProject(config,projectID){var newProject=firebase.initializeApp(config,projectID);return newProject.name}});