import{afterNextRender,html,html$2 as html$1,render,cmsMiddlePageTemplate,request}from"../../src/cms-login.js";class cmsProjects extends cmsMiddlePageTemplate{static get _getSilentAnchor(){return html`            
...
this.spinner.active=!1})}},120)}}/*else if (reset === true) {
        ...
        this._contentChanged()
                        ...
                        </cms-page-list-item>`})} `;render(pageTemplate(data),this)},60)}}customElements.define(cmsProjects.is,cmsProjects);function getProjects(){return _getProjects.apply(this,arguments)}function _getProjects(){_getProjects=babelHelpers.asyncToGenerator(function*(){let proj;yield request("http://127.0.0.1:8001/data/projects.json","GET").then(res=>res.text()).then(res=>{proj=res});return proj});return _getProjects.apply(this,arguments)}function changeProject(config,projectID){var newProject=firebase.initializeApp(config,projectID);return newProject.name}