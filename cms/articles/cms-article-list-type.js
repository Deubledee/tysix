define(["../cms-login.js"],function(_cmsLogin){"use strict";class cmsArticleListItem extends _cmsLogin.cmsItemTemplate{static get template(){return _cmsLogin.html`
    <style>    
    :host {
        position: relative;
        display: block;
    ...
    <slot name="table"></slot>`}static get is(){return"cms-article-list-item"}static get properties(){return{noItem:{type:Array,value:[{image:[]}]},translator:{type:Object,notify:!0,value:function(){return MyAppGlobals.translator}}}}ready(){super.ready()}log(data){console.log("log from cms-article-viewer",data)}_putRow(data){this.translator.template.innerHTML=`
        ...
        </article>`;this.translator.clone(this);this.children[0].children[1].children[0].addEventListener("click",this.showPage.bind(this))}_getPagename(cats){return cats.info[0].id.split("_").join(" ")}_getPublished(cats){return cats}error(data){console.error("error from cms-article-viewer",data)}deSpin(){this.$.spinner.active=!this.$.spinner.active}showPage(){let string=window.btoa(`${JSON.stringify(this.page)}`);window.history.pushState({},null,`${this.rootPath}content/articles/view-articles?content=${string}&type=${this.page.info[0].type}&category=${this.page.info[0].id}`);window.dispatchEvent(new CustomEvent("location-changed"))}__delete(data){let page=data;this.translator._DBW.deletePage(msg=>{if("error"!==msg){this.log(msg)}else{this.error(msg)}},page,this.translator.__DEV)}__publish(){console.log("!!to be done!!")}_openConfirm(){this._changeSectionDebouncer=_cmsLogin.Debouncer.debounce(this._changeSectionDebouncer,_cmsLogin.microTask,()=>{this.dispatchEvent(new CustomEvent("confirm",{bubbles:!0,composed:!0,detail:{name:this._getPagename(this.page),method:this.__delete.bind(this),headderMsg:"delete category page"}}))})}_confirmPublish(){this._changeSectionDebouncer=_cmsLogin.Debouncer.debounce(this._changeSectionDebouncer,_cmsLogin.microTask,()=>{this.dispatchEvent(new CustomEvent("confirm",{bubbles:!0,composed:!0,detail:{name:this._getPagename(this.page),method:this.__publish.bind(this),headderMsg:"publish",type:"category page"}}))})}}customElements.define(cmsArticleListItem.is,cmsArticleListItem);class cmsArticleListType extends _cmsLogin.cmsMiddlePageTemplate{static get _getShoutAnchor(){return _cmsLogin.html`        
        ...
        <div table class="scroll"> 
             ...
             `}static get _getNavside(){return _cmsLogin.html`
